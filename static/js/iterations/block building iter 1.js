let emptyMap;
let inputMap;
let scene;

let objects;

let allowModification;


window.onload  = function () {

    //grid constructor
    const gridLength = 7;

    emptyMap = [];
    var twoDmap = [];
    var oneDmap = [];

    for (let i=0;i<gridLength;i++) {
      oneDmap.push(null);
    }
    for (var i=0;i<gridLength;i++) {
      twoDmap.push(clone(oneDmap));
    }
    for (var i=0;i<gridLength;i++) {
      emptyMap.push(clone(twoDmap));
    }

    const centerOffset = (gridLength / 2 - .5);

    // Canvas
    const canvas = document.querySelector('canvas.webgl');

    // Scene
    scene = new THREE.Scene();

    // Objects
    inputMap = clone(emptyMap);
    wfcMap = clone(emptyMap);
    nullMap = clone(emptyMap);

    for ([x, slice] of emptyMap.entries()) {
        for ([y,row] of slice.entries()) {
            for ([z,tile] of row.entries()) {
                inputMap[x][y][z] = Math.round(Math.random())
                nullMap[x][y][z] = 0
            }
        }
    }
    prevWfcMap = nullMap

    // Sizes and dynamic resizing

    const sizes = {
        width: window.innerWidth,
        height: window.innerHeight
    }
    window.addEventListener('resize', function(e) {
        resize()
    })
    function resize() {
        // Update sizes
        sizes.width = window.innerWidth;
        sizes.height = window.innerHeight;

        // Update camera
        camera.aspect = sizes.width / sizes.height;
        camera.updateProjectionMatrix();
        controls.target.set(centerOffset, centerOffset, centerOffset);

        // Update renderer
        renderer.setSize(sizes.width, sizes.height);
        renderer.setPixelRatio(Math.min(window.devicePixelRatio, 2));
    }


    //Renderer


    const renderer = new THREE.WebGLRenderer({
        canvas: canvas
    });
    renderer.setSize(sizes.width, sizes.height);
    renderer.setPixelRatio(Math.min(window.devicePixelRatio, 2));
    document.body.appendChild(renderer.domElement);

    const raycaster = new THREE.Raycaster();
    const mouse = new THREE.Vector2();


    /**
     * Camera
     */
    // Base camera
    camera = new THREE.PerspectiveCamera (45, window.width/window.height, 1, 10000);
    camera.position.y = 160;
    camera.position.z = 400;
    camera.lookAt (new THREE.Vector3(0,0,0));

    const controls = new THREE.OrbitControls(camera);


    // Clock
    const clock = new THREE.Clock();



    constructMap()
    refreshScene()

    const tick = () => {

        const elapsedTime = clock.getElapsedTime();


        // Update Orbital Controls
        // controls.update()

        // Render
        controls.update();
        //requestAnimationFrame ( animate );
        renderer.render(scene, camera);

        // Call tick again on the next frame
        window.requestAnimationFrame(tick);
    };

    window.addEventListener('mousemove', function () {
        // updates mouse position
        mouse.x = (event.clientX / window.innerWidth) * 2 - 1;
        mouse.y = -(event.clientY / window.innerHeight) * 2 + 1;
    });

    window.addEventListener('mousedown', function (event) {
        raycaster.setFromCamera(mouse, camera);
        // get all objects that intersect the mouse
        var objectIntersects = raycaster.intersectObjects(objects, false);
        // check if the mouse hovers over any objects
        if (objectIntersects.length > 0) {
            // objects are ordered by distance so first object is the hovered over object
            var intersectObject = objectIntersects[0];
            // calculating indices of add/del positions
            delPos = clone(intersectObject.object.position)
            addPos = clone(delPos).add(intersectObject.face.normal)

            switch (event.which) {
                case 1: //left click
                    // check that the position is inside the allowed area
                    if (0 <= addPos.x && 0 <= addPos.y && 0 <= addPos.z) {
                        if (addPos.x <= gridLength && addPos.y <= gridLength && addPos.z <= gridLength) {
                            inputMap[addPos.x][addPos.y][addPos.z] = 1
                        }
                    }
                    break;

                case 2: // middle click
                    break;

                case 3: //right click
                    inputMap[delPos.x][delPos.y][delPos.z] = 0
                    break;

                default:
                    // edge case for unusual mouse buttons
                    alert('unknown mouse button')

            }

            constructMap()
            refreshScene()
        }
    });


    resize()
    tick();
};

//Artificial will not need forever
function constructMap() {
    wfcMap = clone(emptyMap)
    objects = [];
    const geometry = new THREE.BoxGeometry(1, 1, 1);
    const material = new THREE.MeshNormalMaterial()
    // iterate through every cubic space
    for ([x, slice] of emptyMap.entries()) {
        for ([y, row] of slice.entries()) {
            for ([z, tile] of row.entries()) {
                if (inputMap[x][y][z] == 1) {
                    const cube = new THREE.Mesh(geometry, material);
                    cube.position.set(x, y, z)
                    wfcMap[x][y][z] = cube;
                    objects.push(cube);
                }
            }
        }
    }

    //console.log(inputMap)
    //console.log(wfcMap)
}


function refreshScene() {
    // iterate through every cubic space
    for ([x, slice] of emptyMap.entries()) {
        for ([y, row] of slice.entries()) {
            for ([z, tile] of row.entries()) {
                if (prevWfcMap[x][y][z] !== null) {
                    // remove the object from the scene and objects array
                    scene.remove(prevWfcMap[x][y][z])
                    objects = removeItem(objects, prevWfcMap[x][y][z]);
                }
                if (wfcMap[x][y][z] !== null) {
                    // add the object from the scene and objects array
                    scene.add(wfcMap[x][y][z])
                    objects.push(wfcMap[x][y][z]);
                }
            }
        }
    }
    // update the prevWfcMap to the current wfcMap
    prevWfcMap = wfcMap
}


function removeItem (array, removeItem) {
    const index = array.indexOf(removeItem);
    if (index > -1) {
        array.splice(index);
    }
    return array
}


function clone(obj){
    if(obj == null || typeof(obj) != 'object')
        return obj;

    var temp = new obj.constructor();
    for(var key in obj)
        temp[key] = clone(obj[key]);

    return temp;
}